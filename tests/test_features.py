...
class TestFeatures(TestCase):
    ...
    def test_standard_scaler(self):
        ...
        assert np.allclose(transformed.mean(axis=0), 0)  # Mean should be approximately 0
        assert np.allclose(transformed.std(axis=0), 1)   # Std should be approximately 1

    def test_label_encoder(self):
        labels = np.array(['cat', 'dog', 'fish', 'dog', 'cat'])
        encoder = LabelEncoder()
        encoded = encoder.fit_transform(labels)
        
        assert encoder.classes_.tolist() == ['cat', 'dog', 'fish']
        assert (encoded == [0, 1, 2, 1, 0]).all()
